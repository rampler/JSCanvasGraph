...
jsCanvasGraph = {
    ...
    snapToGrid: function(graph, state){ return jsCanvasGraph.Engine.snapToGrid(graph, state); },
...
jsCanvasGraph.Engine = {
    ...
    fitNodeToGrid : function(node) {
        node.x = Math.round(node.x/50)*50;
        node.y = Math.round(node.y/50)*50;
    },
    fitNodesToGrid : function(graph){
        for(var i=0; i<graph.nodes.length; i++)
            this.fitNodeToGrid(graph.nodes[i]);
    },
    snapToGrid: function(graph, state){
        graph.snapToGridMode = state;
        return state;
    },
    ...
    this.canvas.elem.onmousedown = function(e){
        ...
        if(e.button == 0 || e.button == 1) {
            ...
            if (foundNode != null) {
                ...
                if(context.snapToGridMode)
                    jsCanvasGraph.Engine.fitNodeToGrid(draggedNode);
