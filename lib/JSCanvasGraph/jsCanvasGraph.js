...
jsCanvasGraph.Engine = {
    ...
    loadGraph: function(graph, json){
        ...
        for(var i=0; i<json.nodes.length; i++)
            graph.nodes.push(eval('jsCanvasGraph.Engine.Graph.Node.'+json.nodes[i].type+'.load('+JSON.stringify(json.nodes[i])+')'));

        for(var j=0; j<json.edges.length; j++)
            graph.edges.push(eval('jsCanvasGraph.Engine.Graph.Edge.'+json.edges[j].type+'.load('+JSON.stringify(json.edges[j])+','+graph+')'));
...
jsCanvasGraph.Engine.Graph.Node.SimpleNode.load = function(json) {
    var elements = [];
    for(var k=0; k<json.elements.length; k++)
        elements.push(eval('jsCanvasGraph.Engine.Graph.Node.Element.'+json.elements[k].type+'.load('+JSON.stringify(json.elements[k])+')'));
    return new jsCanvasGraph.Engine.Graph.Node.SimpleNode(json.id, json.x, json.y, elements);
};
...
jsCanvasGraph.Engine.Graph.Node.Element.Title.load = function(json) {
    return new jsCanvasGraph.Engine.Graph.Node.Element.Title(json.title);
};
...
jsCanvasGraph.Engine.Graph.Node.Element.Description.load = function(json) {
    return new jsCanvasGraph.Engine.Graph.Node.Element.Description(json.text);
};
...
jsCanvasGraph.Engine.Graph.Node.Element.Figure.load = function(json) {
    return new jsCanvasGraph.Engine.Graph.Node.Element.Figure(json.src);
};
...
jsCanvasGraph.Engine.Graph.Edge.SimpleEdge.load = function(json, graph) {
    //var source = jsCanvasGraph
    console.log(graph);
    return new jsCanvasGraph.Engine.Graph.Edge.SimpleEdge(source, target);
};