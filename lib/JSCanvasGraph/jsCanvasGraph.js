...
jsCanvasGraph.Engine = {
    ...
    createNode : function(graph, x, y){
        var nodeId = jsCanvasGraph.Engine.getNextFreeId(graph);
            ...
            $contextMenu = $('' +
                '<div id="jsCanvasGraph-context-menu" style="position: absolute; top: '+ev.pageY+'px; left: '+ev.pageX+'px; width: 225px;">' +
                    '<div class="list-group">' +
                        '<button type="button" class="list-group-item">Add Node</button> ' +
                        '<button type="button" class="list-group-item">Save as PNG</button> ' +
                    '</div>' +
                '</div>');
            $('body').append($contextMenu);
        }
