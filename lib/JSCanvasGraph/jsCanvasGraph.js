...
jsCanvasGraph.Engine = {
    ...
    fitNodesToGrid : function(graph){
        for(var i=0; i<graph.nodes.length; i++) {
            graph.nodes[i].x = Math.round(graph.nodes[i].x/75)*75;
            graph.nodes[i].y = Math.round(graph.nodes[i].y/75)*75;
        }
    },
    loadGraph: function(graph, json){
        ...
        for(var j=0; j<json.edges.length; j++) {
            ...
            switch(json.edges[j].type) {
                case 'SimpleEdge':
                    edge = new this.Graph.Edge.SimpleEdge(this.findNodeById(graph, json.edges[j].source), this.findNodeById(graph, json.edges[j].target));
                    break;
            }
            graph.edges.push(edge);
        }
...
jsCanvasGraph.Engine.Graph = function(_el, _nodes, _edges) {
    ...
    this.draw = function() {
        ...
        for(var k=0; k<context.canvas.context.canvas.width; k+=75){
            context.canvas.context.moveTo(k,0);
            context.canvas.context.lineTo(k, context.canvas.context.canvas.height);
        }
        for(var h=0; h<context.canvas.context.canvas.width; h+=75){
    ...
    this.canvas.elem.addEventListener('contextmenu', function(ev){
        ...
        if(foundNode) {
                        ...
                        '<div class="panel-body">' +
                            '<form class="form-horizontal">' +
                                '<div class="form-group">' +
                                    ...
                                    '</div>'+
                                    '<label for="jsCanvasGraph-context-menu-fill-color" class="col-sm-2 control-label">Fill color</label>' +
                                    '<div class="col-sm-5">' +
                                ...
                                '</div>'+
                                '<div class="form-group">' +
                                    '<label for="jsCanvasGraph-context-menu-border-width" class="col-sm-2 control-label">Border width</label>' +
                                    '<div class="col-sm-3">' +
                                        '<input type="number" class="form-control" id="jsCanvasGraph-context-menu-border-width" placeholder="Line width" value="'+foundNode.borderLineWidth+'">' +
                                    '</div>'+
                                    '<label for="jsCanvasGraph-context-menu-border-color" class="col-sm-2 control-label">Border color</label>' +
                                    '<div class="col-sm-5">' +
                                ...
                                '</div>'+
                                '<div class="form-group">' +
                                    '<label for="jsCanvasGraph-context-menu-border-width" class="col-sm-2 control-label">Width</label>' +
                                    '<div class="col-sm-3">' +
                                        '<input type="number" class="form-control" id="jsCanvasGraph-context-menu-node-width" placeholder="Node height" value="'+foundNode.width+'">' +
                                    '</div>'+
                                    '<label for="jsCanvasGraph-context-menu-border-width" class="col-sm-2 control-label">Height</label>' +
                                    '<div class="col-sm-5">' +
                                        '<input type="number" class="form-control" id="jsCanvasGraph-context-menu-node-height" placeholder="Node height" value="'+foundNode.height+'">' +
                                    '</div>'+
                                '</div>'+
                                '<div class="form-group">' +
                                    '<div class="col-sm-12">' +
            ...
            $('#jsCanvasGraph-context-menu-node-width').on('input',function(){
                if($(this).val() != "")
                    foundNode.width = parseInt($(this).val());
            });
            $('#jsCanvasGraph-context-menu-node-height').on('input',function(){
                if($(this).val() != "")
            ...
            $('#jsCanvasGraph-context-menu-change').click(function(e){
                e.preventDefault();
