...
jsCanvasGraph.Engine.Graph = function(_el, _nodes, _edges) {
    ...
    this.canvas.elem.onmousedown = function(e){
        for(var i=0; i<context.nodes.length; i++) {
            if(e.pageX < context.nodes[i].x + context.nodes[i].width + $(context.canvas.elem).offset().left)
                if(e.pageX > context.nodes[i].x + $(context.canvas.elem).offset().left)
                    if(e.pageY < context.nodes[i].y + context.nodes[i].height + $(context.canvas.elem).offset().top)
                        if(e.pageY > context.nodes[i].y + $(context.canvas.elem).offset().top){
                            draggedNode = context.nodes[i];
